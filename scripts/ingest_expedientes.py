...
def _cmd_export(args: argparse.Namespace) -> int:
    ...
    for exp_dir in exp_dirs:
        ...
        for c in comprobantes_exp:
            # PRE-PASO 4.5 (D-24): los 3 campos vienen ya calculados desde el
            # consolidador y persistidos en expediente.json. El exportador solo
            # lee. flag_revision_manual sigue siendo lógica de presentación
            # (qué fila se pinta roja), no de modelo.
            estado_cons = c.get("estado_consistencia", "")
            tipo_trib = c.get("tipo_tributario", "")
            detalle_cons = c.get("detalle_inconsistencia", "")
